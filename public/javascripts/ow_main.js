...
function list(data){
		...
		for(var i=0;i<data.length-1;i++){
			$('ul li:nth-child('+(i+1)+')').after("<li class='list'><p>"+data[i].table_no+"</p><p>"+
...
function specific(){
	$('.specific').on('click',function(time){
		...
		$.ajax({
				url:"http://127.0.0.1:3000/ownerMain/specific?time="+time,
				success:function(data){
...
$('#save').on('click',function(){
	...
	$('.list').each(function(){
		...
		if($(this).children().first().next().next().next().children().first().children().is( ":checked" ))
			arr.push('oshow');
		else
			arr.push('noshow');
	});
